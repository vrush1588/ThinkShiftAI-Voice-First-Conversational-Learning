...
│   ├── vision_client.py          Gemini: describes homework photos
...
└── frontend_mobile/              Flutter app (Android target)
    ├── pubspec.yaml              Dependencies: agora_rtc_engine, http, permission_handler, image_picker
    ├── lib/
    ...
    │   ├── thinkshift_api.dart   HTTP client for /token, /start-agent, /stop-agent, /analyze-homework
...
| [agora_client.py](backend/agora_client.py) | Runs a background asyncio loop for the async SDK, builds the agent, starts/stops/interrupts agents, sends `think` instructions, generates tokens | `constants`, `agora-agents`, `agora-token-builder`, `httpx`, `requests` |
| [vision_client.py](backend/vision_client.py) | Sends a homework photo to Gemini and returns a short spoken-style description | `constants`, `google-genai` |
...
* How homework photos are described and discussed → `VISION_PROMPT`, `HOMEWORK_THINK_TEMPLATE`, `GEMINI_VISION_MODEL` in `constants.py`
...
| [thinkshift_api.dart](frontend_mobile/lib/thinkshift_api.dart) | `getToken`, `startAgent`, `stopAgent`, `analyzeHomework` (multipart upload) wrappers with timeouts |
| [voice_screen.dart](frontend_mobile/lib/voice_screen.dart) | `VoiceState` machine (`idle → connecting → joining → startingAgent → listening → ending`), RTC engine setup and events, and the UI (AI orb, wave bars, suggestion pills, mic button, camera button for homework photos) |
